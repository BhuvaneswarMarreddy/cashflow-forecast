...
EXPECTED_PEOPLE = {
    "REMITLY": (12056236, 0),
    "PRIYA EXAMPLE": (1861200, 0),
    "ARJUN EXAMPLE": (238000, 1778800),
...
EXPECTED_RECURRING = {
    # occurrences = unique charge DAYS (rule 1): 21 payments, two on 2025-12-01 -> 20 days
...
maybe('audit replay over the real CSVs', () => {
  ...
  it('finds the audited people totals (±$1)', () => {
    ...
    const priya = linkSum((id) => id === 'person-out:PRIYA EXAMPLE', 'target');
    expect(Math.abs(priya - toCents(18612))).toBeLessThanOrEqual(100);
  });
...
uppercase (`person-out:PRIYA EXAMPLE`); if this test can't find a node, print
