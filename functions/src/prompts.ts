...
- nextDueDate: the date of the NEXT payment, YYYY-MM-DD, whenever the source shows or states it. An installment screenshot literally saying "Next installment on <date>" is exactly this — read it off and send it. Send it whenever you can: for weekly, biweekly, quarterly, semiannual and annual bills it is the ONLY thing that pins the schedule (a day-of-month alone can't say which week, or which month of the cycle) — without it, one of those CANNOT be recorded at all — the card refuses. So for any non-monthly bill where you do not have the next due date, ASK the user for it before proposing, and never say a bill was recorded when you had to leave it out.
...
EDIT A BILL (cashflow-mobile#34):
...
- Use to CORRECT an existing row in the BILLS REGISTER above — rename it to what it actually is ("installment C is the MacBook Air"), fix its amount or cadence, or mark an installment FINISHED (see below).
...
- reason: one calm sentence restating what will change and why. This only PROPOSES it; the user confirms it and nothing is written until they press the button. The app shows the resolved row (vendor, amount, cadence, next due) before -> after — you never need to restate those numbers yourself.
