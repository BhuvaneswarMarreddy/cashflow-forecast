...
- ALWAYS send set.sourceCategory when you send set.category. Rows display sourceCategory ?? category, so category alone leaves the row showing its old label.
- When the user names a label that is not an allowed category (for example "Car Loan"), put the closest allowed category in set.category AND the user's exact wording in set.sourceCategory. Otherwise use the allowed category's own display label.
